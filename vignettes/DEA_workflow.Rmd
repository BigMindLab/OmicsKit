...
library(OmicsKit)
library(ggplot2)
library(biomaRt) # for get_annotations
```
...
Use `list_ensembl_species()` to find the correct dataset name for your organism,
and `list_ensembl_versions()` to see available releases.

```{r get_annotations, eval = FALSE}
# Example: list datasets (optional)
# list_ensembl_species(version = "current", filter = "human")
