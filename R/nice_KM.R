                    ...
                    returnData = FALSE)
{
  ...
  if (length(counts) < 2 || any(counts == 0)) {
    warning("Only one category in ", gene)
    empty_plot <- ggplot() +
      theme_void() +
      annotate(
        "text", x = .5, y = .5, hjust = .5, size = 6,
        label = paste0("Only one\ncategory in\n", sub("_muts$", "", gene))
      )
  ...
  km_plot <- km_out$plot +
    theme(
      ...
      axis.title.x = element_text(size = axis_title_size["x"]),
      axis.title.y = element_text(size = axis_title_size["y"]),
      axis.title = element_text(size = title_size["axis"]),
      axis.text.x = element_text(size = axis_text["x"]),
      axis.text.y = element_text(size = axis_text["y"]),
      legend.title = element_text(size = legend_text["title"]),
      legend.text = element_text(size = legend_text["labels"]),
      legend.position.inside = legend_pos,
      legend.direction = "horizontal"
    )

  if (returnData) {
    return(list(km_fit = km_fit, plot = km_plot))
  }
