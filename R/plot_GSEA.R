...
#' @return GSEA barplots arranged in a grid.
#' @export

plot_GSEA <- function(data, geneset_col, collection_col, nes_col, logfdr_col,
                      text_size_genesets = 5, text_size_collection = 5)
{

  if (!requireNamespace("patchwork", quietly = TRUE)) stop("Package \"patchwork\" must be installed to use this function.", call. = FALSE)
  if (!requireNamespace("cowplot", quietly = TRUE)) stop("Package \"cowplot\" must be installed to use this function.", call. = FALSE)
  ...
  plot_legend <- ggplot(data, aes(x = NES, y = Geneset, fill = logFDR)) +
    ...
    theme(legend.position = "right",
          ...
          legend.key.size = grid::unit(1.5, "cm"),  # Bigger color key size
          legend.key.height = grid::unit(2, "cm"),  # Increase the height of the legend box
          legend.spacing = grid::unit(3.5, "cm"),  # More space between title and legend
  ...
  final_plot <- plot_text_pathways + plot_left +  plot_center + plot_right + plot_text_msigdb + plot_right_legend +
    patchwork::plot_layout(ncol = 6, widths = c(4, 25, 15, 3, 10, 3))
