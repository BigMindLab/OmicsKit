...
#' @param reference A reference table with the annotations. Default: geneID.details in our custom pipeline.
#' @param variables Annotations (columns) from the reference table to add, i.e. gene symbol, gene_length, description.
#' @export

add_annotations <- function(object, reference, variables = NULL){
  ...
  for(i in 1:length(variables)){
    df[,variables[i]] <- reference[index, variables[i]]
  }
